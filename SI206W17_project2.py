...
unique_tw_identifier = "twitter_University of Michigan"
def write_tw():
    if unique_tw_identifier in CACHE_DICTION:
        print('using cached data for twitter')
    else:
        print('getting data from Twitter')
        twitter_results = api.search(q = unique_tw_identifier)
        CACHE_DICTION[unique_tw_identifier] = twitter_results
        f = open(CACHE_FNAME, 'w')
        f.write(json.dumps(CACHE_DICTION))
        f.close()
    return CACHE_DICTION[unique_tw_identifier]
write_tw()
...
def find_urls(string):
    url = re.findall(r'((http://|https://)([a-z]+\.)?\w+(\.[a-z][a-z]+)+)', string)
    url_ls = []
    for i in url:
        url_ls.append(i[0])
    return url_ls
...
def get_umsi_data():
    ...
    if unique_identifier in CACHE_DICTION:
        html_txt = CACHE_DICTION[unique_identifier]
    else:
        base_url = "https://www.si.umich.edu/directory"
        para_dict = {}
        para_dict["field_person_firstname_value"] = ""
        para_dict["field_person_lastname_value"] = ""
        para_dict["rid"] = "All"
        resp = requests.get(base_url, params=para_dict, headers={'User-Agent': 'SI_CLASS'}).text
        html_txt = []
        html_txt.append(resp)
        for page in range(1, 12):
            para_dict["page"] = page
            resp = requests.get(base_url, params=para_dict, headers={'User-Agent': 'SI_CLASS'}).text
            html_txt.append(resp)
        CACHE_DICTION[unique_identifier] = html_txt
        f = open(CACHE_FNAME, 'w')
        f.write(json.dumps(CACHE_DICTION))
        f.close()
    return html_txt
...
def umsi_titles():
    name_dict = {}
    html_txt = get_umsi_data()
    for page in html_txt:
        soup = BeautifulSoup(page, "html.parser")
        people = soup.find_all("div", {"class": "views-row"})
        for i in people:
            name = i.find(class_ =  "field field-name-title field-type-ds field-label-hidden").text
            tile = i.find(class_ =  "field field-name-field-person-titles field-type-text field-label-hidden").text
            name_dict[name] = tile
    return name_dict
...
def get_five_tweets(string):
    tw_list = []
    five_tw = []
    if string == unique_tw_identifier:
        res = CACHE_DICTION[string]
        for tw in res["statuses"]:
            tw_list.append(tw["text"])
        for i in range(1, 6):
            five_tw.append(tw_list[i])
    else:
        res = api.search(q = string)
        for tw in res["statuses"]:
            tw_list.append(tw["text"])
        for i in range(1,6):
            five_tw.append(tw_list[i])
        CACHE_DICTION[unique_tw_identifier] = res
        f = open(CACHE_FNAME, 'w')
        f.write(json.dumps(CACHE_DICTION))
        f.close()
    return  five_tw
...
tweet_urls_found = []
for tw in five_tweets:
    url_ls = find_urls(tw)
    if len(url_ls) > 0:
        for i in range(0, len(url_ls)):
            tweet_urls_found.append(url_ls[i])
...
class CachingTests(unittest.TestCase):
    ...
    def test_cache_file(self):
        ...
        self.assertEqual(type(s), type(""),
                         "Doesn't look like you have a cache file with the right name / with content in it")
    ...
    def test_cache_data(self):
        ...
        f.close()
        jv = json.loads(s)
        self.assertTrue("umsi_directory_data" in jv,
                        "Checking whether the UMSI data identifier is in the data in the cache file, formatted properly")
    ...
    def test_cache_data2(self):
        ...
        f.close()
        jv = json.loads(s)
        self.assertTrue("twitter_University of Michigan" in jv,
                        "Checking whether the Twitter University of Michigan identifier is correctly in the cache")
...
class PartOne(unittest.TestCase):
    ...
    def test_findurls_multurls(self):
        self.assertEqual(
            find_urls("I love looking at websites like http://etsy.com and http://instagram.com and lol.com and stuff"),
            ["http://etsy.com", "http://instagram.com"])

    def test_findurls_multurls_all(self):
        self.assertEqual(find_urls(
            "I love looking at websites like http://etsy.com and http://instagram.com and https://www.bbc.co.uk and stuff"),
                         ["http://etsy.com", "http://instagram.com", "https://www.bbc.co.uk"])
